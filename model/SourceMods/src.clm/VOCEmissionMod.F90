module VOCEmissionMod
  ...
  type :: gamma_out_type
     ! Compound-specific activity factors for CLM history output.
     ! Each pointer is indexed over patches (begp:endp).
     real(r8), pointer, private :: gamma (:) ! total activity factor
     real(r8), pointer, private :: gammaP(:) ! light / PPFD activity factor
     real(r8), pointer, private :: gammaT(:) ! temperature activity factor
     real(r8), pointer, private :: gammaA(:) ! leaf-age activity factor
  end type gamma_out_type
  type(gamma_out_type), private, pointer :: gam_out(:) ! (n_megan_comps)

  type :: epsilon_out_type
     ! Baseline emission factor actually used, including mapped isoprene EF.
     real(r8), pointer, private :: epsilon(:) ! [ug m-2 h-1], indexed over patches
  end type epsilon_out_type
  type(epsilon_out_type), private, pointer :: eps_out(:) ! (n_megan_comps)
  !
...
contains
  ...
  subroutine InitAllocate(this, bounds)
    ...
    allocate(gam_out(shr_megan_megcomps_n))
    do i=1,shr_megan_megcomps_n
       allocate(gam_out(i)%gamma (begp:endp)); gam_out(i)%gamma (:) = nan
       allocate(gam_out(i)%gammaP(begp:endp)); gam_out(i)%gammaP(:) = nan
       allocate(gam_out(i)%gammaT(begp:endp)); gam_out(i)%gammaT(:) = nan
       allocate(gam_out(i)%gammaA(begp:endp)); gam_out(i)%gammaA(:) = nan
    end do

    allocate(eps_out(shr_megan_megcomps_n))
    do i=1,shr_megan_megcomps_n
       allocate(eps_out(i)%epsilon(begp:endp)); eps_out(i)%epsilon(:) = nan
    end do
  ...
  subroutine InitHistory(this, bounds)
    ...
    if (shr_megan_megcomps_n>0) then
       ...
       do while(associated(meg_cmp))
          ...
          gam_out(imeg)%gamma (begp:endp) = spval
          gam_out(imeg)%gammaP(begp:endp) = spval
          gam_out(imeg)%gammaT(begp:endp) = spval
          gam_out(imeg)%gammaA(begp:endp) = spval
          eps_out(imeg)%epsilon(begp:endp) = spval

          call hist_addfld1d (fname='GAMMA_'//trim(meg_cmp%name), units='1', &
               avgflag='A', long_name='total VOC activity factor: '//trim(meg_cmp%name), &
               ptr_patch=gam_out(imeg)%gamma, set_lake=0._r8, default='inactive')
          call hist_addfld1d (fname='GAMMAP_'//trim(meg_cmp%name), units='1', &
               avgflag='A', long_name='light activity factor: '//trim(meg_cmp%name), &
               ptr_patch=gam_out(imeg)%gammaP, set_lake=0._r8, default='inactive')
          call hist_addfld1d (fname='GAMMAT_'//trim(meg_cmp%name), units='1', &
               avgflag='A', long_name='temperature activity factor: '//trim(meg_cmp%name), &
               ptr_patch=gam_out(imeg)%gammaT, set_lake=0._r8, default='inactive')
          call hist_addfld1d (fname='GAMMAA_'//trim(meg_cmp%name), units='1', &
               avgflag='A', long_name='leaf-age activity factor: '//trim(meg_cmp%name), &
               ptr_patch=gam_out(imeg)%gammaA, set_lake=0._r8, default='inactive')
          call hist_addfld1d (fname='EPS_'//trim(meg_cmp%name), units='ug/m2/h', &
               avgflag='A', long_name='baseline emission factor used: '//trim(meg_cmp%name), &
               ptr_patch=eps_out(imeg)%epsilon, set_lake=0._r8, default='inactive')
       ...
       call hist_addfld1d (fname='GAMMAL', units='1',  &
            avgflag='A', long_name='LAI activity factor (all MEGAN compounds)', &
            ptr_patch=this%gammaL_out_patch, set_lake=0._r8, default='inactive')

       this%gammaS_out_patch(begp:endp)  = spval
       call hist_addfld1d (fname='GAMMAS', units='1',  &
            avgflag='A', long_name='soil-moisture activity factor (all MEGAN compounds; currently fixed at 1)', &
            ptr_patch=this%gammaS_out_patch, set_lake=0._r8, default='inactive')

       this%gammaC_out_patch(begp:endp)  = spval
       call hist_addfld1d (fname='GAMMAC_isoprene', units='1',  &
            avgflag='A', long_name='CO2 inhibition activity factor (isoprene only)', &
  ...
  subroutine VOCEmission (bounds, num_soilp, filter_soilp, &
    ...
    do imeg=1,shr_megan_megcomps_n
      ...
      gam_out(imeg)%gamma (bounds%begp:bounds%endp) = spval
      gam_out(imeg)%gammaP(bounds%begp:bounds%endp) = spval
      gam_out(imeg)%gammaT(bounds%begp:bounds%endp) = spval
      gam_out(imeg)%gammaA(bounds%begp:bounds%endp) = spval
      eps_out(imeg)%epsilon(bounds%begp:bounds%endp) = spval
    enddo
    ...
    do fp = 1,num_soilp
       ...
       if (patch%itype(p) > 0) then 
          ...
          meg_cmp_loop: do while(associated(meg_cmp))
             ...
             eps_out(imeg)%epsilon(p) = epsilon
             ...
             if ( (gamma >=0.0_r8) .and. (gamma< 100._r8) ) then
                ...
                gam_out(imeg)%gamma (p) = gamma
                gam_out(imeg)%gammaP(p) = gamma_p
                gam_out(imeg)%gammaT(p) = gamma_t
                gam_out(imeg)%gammaA(p) = gamma_a
